...
The released Dex Go SDK `v0.4.0` provides `WaitForAttributeMatch`, read-only
Flow RPCs, and exact AttributeMap instance loading. Streams remain disposable
latency hints.
...
invokes one read-only Flow RPC that loads exactly one ten-message instance by
`beforeSequence`. It reads `AgentState` only to enforce retention and derive the
next boundary. Retention deletes only complete archived chunks already covered
by a committed summary.
...
Channels. Older history costs one exact archive RPC per top-scroll action.
