...
def create_custom_retry_blueprint(app_state: ExampleApp) -> Blueprint:
    blueprint = Blueprint(
        "primitive_custom_retry", __name__, url_prefix="/primitives/step/custom-retry"
    )
    ...
    async def start() -> Response:
        ...
        run_id = await app_state.client.start_flow(
            app_state.custom_retry,
            flow_id,
            required_int_query("readyAfterAttempt"),
            start_options(),
        )
