...
def create_subflow_blueprint(app_state: ExampleApp) -> Blueprint:
    blueprint = Blueprint("primitive_subflow", __name__, url_prefix="/primitives/subflow")

    @blueprint.get("/start")
    async def start() -> Response:
        flow_id = required_query("workflowId")
        run_id = await app_state.client.start_flow(
            app_state.subflow_parent,
            flow_id,
